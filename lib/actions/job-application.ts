...
//     verify column belongs to board
    ...
    const maxOrder = (await JobApplication.findOne({columnId}).sort({order: -1}).select("order").lean()) as {
        order: number
    } || null;
    const jobApplicationDoc = await JobApplication.create({
        company,
        position,
        location,
        notes,
    ...
    await Column.findByIdAndUpdate(columnId, {
        $push: {jobApplications: jobApplicationDoc._id}
    });
    revalidatePath("/dashboard");
    const jobApplication = JSON.parse(JSON.stringify(jobApplicationDoc));
